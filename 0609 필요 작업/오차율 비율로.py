...
cur = con.cursor()
sql = "SELECT pk, 시도, official_price, 경도, 위도, closest_sub, closest_high, school_1km, area, floor, actual FROM off_act_price WHERE 시도='경기도'"
cur.execute(sql)
rows = cur.fetchall()
con.close()
...
print("경기도 import 실행 소요시간 (단위 초) : " + taketime)
...
plt.title('Proportion of Errors in Gyeonggido')
